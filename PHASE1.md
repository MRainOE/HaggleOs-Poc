...
- Click the HaggleOS icon to open/focus the dashboard (defaults to `http://localhost:3000/dashboard` unless `haggleosSettings.api_base_url` is set in storage).

## Bridge (extension <-> web)
- A content script (`dashboard_bridge.js`) runs on `http://localhost:3000/dashboard*`.
...
1) Click extension icon -> new/existing tab opens `http://localhost:3000/dashboard` (or the URL in `haggleosSettings.api_base_url` if set).
